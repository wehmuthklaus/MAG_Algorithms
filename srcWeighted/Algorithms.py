...
def AdjMatrixToList(J_H):
   AdjList = {x:J_H.getrow(x).indices for x in range(J_H.shape[0]) if len(J_H.getrow(x).indices) > 0}
   return AdjList

def AdjMatrix(H,W):
   """ Returns the Adjacency matrix and companion tuple of a MAG
   :input: MAG H = (A,E) and Weights dictionary
   ...
   for e in E:
      ...
      Wn[tuple((u,v))] = W[e]		# get weights
   return J_H.tocsr(), T, Wn
...
def BFS_Sub(J_H,T_H,zeta,s):
   ...
   for v in range(n):			# for every v in V(H)
      if J_H.getrow(v).nnz > 0:		# if v has out edges
         v = InvD(v+1, T_H)		# v in numeric tuple form
...
def Dijkstra(AdjList,T,W,startT,endT,zetaE):
   D = {}	# dictionary of final distances
   P = {}	# dictionary of predecessors
   Q = priority_dict()   # est.dist. of non-final vert.
   Te = SubCompTuple(T, zetaE)
   end = subDetVertex(endT-1, T, Te)
   Dend = float('inf')
   Vend = 0
   ...
   while len(Q) > 0:
      ...
      vNeigbors = AdjList[v]
      for u in vNeigbors:
         vuLength = D[v] + W[(v+1,u+1)]
         if (u not in Q or vuLength <= Q[u]):
            Q[u] = vuLength
            if u not in P:
               P[u] = [v]
            else:
...
def buildPath(P,start,end,A_H):
   ...
   while(has_path):
      ...
      has_path = False
      e = end-1
      pth.append(sup.adjMatrixRowToCv(e+1, A_H))
      while(True):
         if len(P[e]) > 1:
            e = P[e].pop()
            has_path = True
...
def subDetVertex(v, T, Tz):
   """ Sub-determine a numerical composite vertex
   :input composite vertex v in its numerical form, the MAG's companion tuple and the sub-determined companion tuple 
   :output the sub-deternimed vertex in numerical form
   """ 
   return D(InvD(v+1, T), Tz)
